...
# URLs to fetch from
JSON_QUOTES_URL = "https://api.coingecko.com/api/v3/simple/price?ids=baked-token&vs_currencies=usd"
...
while True:
    ...
    baked_price = a['baked-token']['usd']
    baked_price = ("%.4f" % baked_price)

    print('This is BAKED price: ' + str(baked_price))
    string = str(baked_price)
    ...
    try:
        for y, row in enumerate(BAKED):
            for x, c in enumerate(row):
                display5.pixel(x + 0, y + 0, c)
    except:
        print('problem with BAKED')
