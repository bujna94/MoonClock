...
# URLs to fetch from
JSON_QUOTES_URL = "https://api.coingecko.com/api/v3/simple/price?ids=polkadot&vs_currencies=usd"
...
while True:
    ...
    polkadot_price = a['polkadot']['usd']
    polkadot_price = ("%.3f" % polkadot_price)

    print('This is POLKADOT price: ' + str(polkadot_price))
    string = str(polkadot_price)
    ...
    try:
        for y, row in enumerate(POLKADOT):
            for x, c in enumerate(row):
                display5.pixel(x + 0, y + 0, c)
    except:
        print('problem with POLKADOT')
